...
exports.lookupMessages = function (status, callback) {
    ...
    var processMsgFile = function (fileHandle) {

        buffer = Buffer.alloc(14);
        fs.read(fileHandle, buffer, 0, 14, 0, function () {
    ...
    fs.open(__dirname + "/firebird.msg", 'r', function (err, h) {
        ...
        if (!h) {
            console.log(`Firebird.MSG File could not be read in ${__dirname + "/firebird.msg"}`);

            var secondPath = `./node_modules/node-firebird/lib/firebird.msg`;
            
            fs.open(secondPath, 'r', function (err2, h2) {

                if(!h2) {
                    console.log(`Firebird.MSG File could not be read in ${secondPath}`);
                    callback();
                    return;
                } else {
                    processMsgFile(h2);
